...
type InvoicesArchiveMonthsNavProps = {
    months: ArchiveMonthItem[];
};

export default function InvoicesArchiveMonthsNav({
    months,
}: InvoicesArchiveMonthsNavProps) {
                ...
                {months.length === 0 ? (
                    ...
                    months.map((m) => (
                        <Button
                            ...
                            className={
                                m.active
                                    ? "cursor-pointer border-0 bg-gradient-to-r from-cyan-500 to-blue-600 text-white hover:from-cyan-500/90 hover:to-blue-600/90 disabled:cursor-not-allowed"
                                    : "cursor-pointer transition hover:bg-slate-100 disabled:cursor-not-allowed"
                            }
                            onClick={() => goTo(m.href)}
                        >
                            {m.label}
